...
class AudioMasteringService:
    ...
    def master(
        ...
        # 2. Apply Gain and True Peak Limiting
        ...
        for s in samples:
            ...
            if prof.limiter_enabled:
                # Soft-knee limiting function: tanh compression for peaks exceeding threshold
                if abs(amplified) > max_peak_linear:
                    sign = 1.0 if amplified > 0 else -1.0
                    excess = abs(amplified) - max_peak_linear
                    compressed = max_peak_linear + (1.0 - max_peak_linear) * math.tanh(excess)
                    amplified = sign * min(compressed, 0.999)
        ...
        output_peak_dbtp = 20.0 * math.log10(max_peak + 1e-12)
        ...
        return {
            ...
            "target_lufs": prof.target_lufs,
            "true_peak_dbtp": round(output_peak_dbtp, 2),
