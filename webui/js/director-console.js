...
async function renderDirectorDelivery(content) {
  ...
  try {
    ...
    const masterLabel = masterVerified ? 'VERIFIED' : masterStatus?.exists && !masterStatus?.fresh ? 'STALE' : 'UNVERIFIED';
    const masterApproval = masterVerified ? '<button onclick="approveDirectorGate(true)" class="px-3 py-2 rounded bg-emerald-600 text-white text-xs font-bold">Approve Final Master</button>' : '<span class="px-3 py-2 rounded bg-[#231F2A] text-slate-500 text-xs">Approval disabled</span>';
    const masterCard = gate ? `<div class="mb-4 p-4 rounded-lg border border-amber-500/50 bg-amber-950/20"><div class="flex flex-wrap justify-between gap-3"><div><h4 class="font-bold text-white">master.wav</h4><p class="text-[10px] text-slate-400 font-mono break-all">SHA-256 ${escapeHtml(gate.sha256)}</p><p class="text-[10px] text-slate-400">Duration ${mixPlan?.duration_ms ? `${Math.round(mixPlan.duration_ms)} ms` : 'not reported'} · Loudness ${metrics.output_lufs ?? 'not reported'} LUFS · Lineage <span class="${masterVerified ? 'text-emerald-400' : 'text-amber-400'}">${masterLabel}</span></p></div>${masterApproval}</div><audio controls preload="metadata" class="w-full h-9 mt-3" src="${escapeHtml(gate.download_url)}"></audio></div>` : '';
