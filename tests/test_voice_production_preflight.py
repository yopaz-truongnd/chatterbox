...
class TestVoiceProductionPreflight(unittest.TestCase):
    ...
    def test_preflight_warns_on_low_disk_space(self):
        ...
        with mock.patch("shutil.disk_usage", return_value=fake_stat), \
             mock.patch("services.local_runtime_service.is_model_cached", return_value=True):
    ...
    def test_missing_local_model_returns_structured_error(self):
        ...
        with mock.patch("services.local_runtime_service.is_model_cached", return_value=False), \
    ...
    def test_selected_local_model_must_be_cached(self):
        ...
        with mock.patch("services.local_runtime_service.is_model_cached", side_effect=cached), \
             mock.patch("services.local_runtime_service._get_job_manager", return_value=mock.Mock(_running=True)):
            issues = self.runtime_service.run_production_preflight(
