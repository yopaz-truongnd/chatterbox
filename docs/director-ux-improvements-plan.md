...
| B3 | Per-beat render progress detail | Director Console | DONE | No provider (chatterbox_job/http/gemini) threaded beat identity through its own progress_callback -- only FakeTTSProvider did. Wrapped the callback at the one place in services/voice_renderer.py that actually knows which beat/attempt is in flight, injecting beat_id and a "Doan {id} ({i}/{n}) . lan thu {k}" message (folded into the existing `message` field rather than widening VoiceProjectOperation's schema). Frontend already had unused beat_id/message display wiring in renderDirectorOperations -- just needed the data and a fingerprint fix so beat changes trigger a re-render. Verified live with a real 2-beat local-provider render: job panel showed "Doan B01 (1/2) . lan thu 1" then correctly advanced to "Doan B02 (2/2) . lan thu 1". Explicitly NOT an ETA subsystem, per the plan's non-goals. |
...
- 2026-09-16: B3 done and verified live with a real 2-beat local-provider render. All 7 planned items complete.
